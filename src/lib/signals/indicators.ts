...
// Input: DailyBar[] newest-first (as returned by fetchDailyClosesStooq).
// No DB access, no API calls — just math.

import type { DailyBar } from "@/lib/market/stooq";
...
export function computeSignals(
  ...
  const prices = [...bars].reverse().map((b) => b.close);
  const highs  = [...bars].reverse().map((b) => b.high);
  const lows   = [...bars].reverse().map((b) => b.low);
  ...
  const return_30d_val = bars.length > 30 ? ((price - bars[30].close) / bars[30].close) * 100 : null;
