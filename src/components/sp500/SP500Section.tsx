...
import { startOfWeek, endOfWeek, isWithinWeek, isWithinNextWeek } from "@/lib/sp500/weekHelpers";
...
function TradeCard({ r, rank }: { r: SP500CompanyRow; rank: number }) {
  const changedAgo = r.signal_changed_at
    ? Math.round((Date.now() - new Date(r.signal_changed_at).getTime()) / 86_400_000)
    : null;
  const daysToEarnings = r.report_date
    ? Math.round((new Date(r.report_date).getTime() - Date.now()) / 86_400_000)
    : null;
  const earningsWarning = daysToEarnings != null && daysToEarnings >= 0 && daysToEarnings <= 10;
...
function WatchRow({ r }: { r: SP500CompanyRow }) {
...
function DataTable({ title, rows, highlightSymbols }: { title: string; rows: SP500CompanyRow[]; highlightSymbols?: Set<string> }) {
  return (
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr className="text-left">
              ...
              <th className="px-3 py-2 text-right">Index weight</th>
              <th className="px-3 py-2 text-right">5D</th>
              <th className="px-3 py-2 text-right">30D</th>
              <th className="px-3 py-2 text-right">vs 200d MA</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
...
function MoversTable({ title, rows, highlightSymbols }: { title: string; rows: SP500CompanyRow[]; highlightSymbols: Set<string> }) {
  return (
    <div className="rounded-lg border bg-white p-4">
      <div className="flex items-baseline justify-between">
        <h2 className="text-base font-semibold">{title}</h2>
...
function ValuationTable({ title, rows }: { title: string; rows: SP500CompanyRow[] }) {
  return (
    <div className="rounded-lg border bg-white p-4">
      <div className="flex items-baseline justify-between">
        <h2 className="text-base font-semibold">{title}</h2>
...
// ── Reporting filter client component ────────────────────────────────────────

function ReportingFilter({ rows }: { rows: SP500CompanyRow[] }) {
...
export function SP500Section({ rows, indexValue, indexChangePct, lastSyncTime }: { rows: SP500CompanyRow[]; indexValue: number | null; indexChangePct: number | null; lastSyncTime?: string }) {
  ...
  const thisWeek = useMemo(
    () => rows.filter((r) => inRange(r.report_date, thisWkStart, thisWkEnd)),
    [rows]
  );
  const nextWeek = useMemo(
    () => rows.filter((r) => inRange(r.report_date, nextWkStart, nextWkEnd)),
    [rows]
  );

  const reportingThisOrNext = useMemo(
    () => new Set([...thisWeek, ...nextWeek].map((r) => r.symbol)),
    [thisWeek, nextWeek]
  );

  function top10ByWeight(rr: SP500CompanyRow[]) {
  ...
  const sectorPerf = useMemo(() => {
    const sectorMap = new Map<string, { r5: number[]; r30: number[] }>();
    for (const r of rows) {
      if (!r.sector) continue;
      if (!sectorMap.has(r.sector)) sectorMap.set(r.sector, { r5: [], r30: [] });
      const s = sectorMap.get(r.sector)!;
      if (r.return_5d != null) s.r5.push(r.return_5d);
      if (r.return_30d != null) s.r30.push(r.return_30d);
    }
    return [...sectorMap.entries()]
      .map(([sector, d]) => ({ sector, ret5d: avg(d.r5), ret30d: avg(d.r30) }))
  ...
  return (
    <div>
      ...
      <div className="mt-4">
          ...
          <div>
            <div className="grid grid-cols-4 gap-3">
              <div className="rounded-lg border bg-green-50 p-4">
                <div className="text-xs font-medium text-green-700">Bullish <InfoTip text="EMA and DMI both pointing up." /></div>
                <div className="mt-1 text-2xl font-bold text-green-800">{pulseBull}</div>
                <div className="text-xs text-green-600">{rows.length > 0 ? Math.round((pulseBull / rows.length) * 100) : 0}% of index</div>
              </div>
              <div className="rounded-lg border bg-red-50 p-4">
                ...
                <div className="text-xs text-red-600">{rows.length > 0 ? Math.round((pulseBear / rows.length) * 100) : 0}% of index</div>
              </div>
              <div className="rounded-lg border bg-gray-50 p-4">
                ...
                <div className="text-xs text-gray-500">{rows.length > 0 ? Math.round((pulseNeutral / rows.length) * 100) : 0}% of index</div>
              </div>
            ...
            <div className="mt-6">
              ...
              <div className="mt-3"><AIAssistant apiPath="/api/ai/sp500" /></div>
            </div>

            <div className="mt-8">
              <h2 className="text-sm font-semibold text-gray-700">Sector performance (average returns across all S&amp;P 500 companies)</h2>
              <div className="mt-3 overflow-auto rounded border">
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr className="text-left">
                  ...
                  {top3Reporting.map((r, i) => {
                    ...
                    return (
                      <div key={r.symbol} className="flex-1 min-w-48 rounded-lg border border-blue-100 bg-blue-50 p-4">
                        ...
                        <div className="mt-1 text-xs text-blue-500">{r.sector ?? "—"}{r.report_date && <span className="ml-2 font-medium">{r.report_date}</span>}</div>
                        {r.signal_label && (
        ...
        {activeTab === "Signals" && (
          <div>
            <div className="mb-2 text-sm text-gray-600">MACD / EMA / DMI signals for all S&amp;P 500 companies</div>
