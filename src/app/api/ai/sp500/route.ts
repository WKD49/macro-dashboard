...
function sectorAggregates(all: any[]) {
  const map = new Map<string, { r5: number[]; r30: number[] }>();
  for (const r of all) {
    const sector = (r.sector ?? "Unknown") as string;
    if (!map.has(sector)) map.set(sector, { r5: [], r30: [] });
...
export async function POST(req: Request) {
  try {
    ...
    const { data: rows, error } = await supabase
      ...
      .select("symbol,name,sector,report_date,eps_actual,eps_estimate,return_5d,return_30d,index_weight")
      .limit(503);
    ...
    const topByWeight = [...nextWeek]
      ...
      .slice(0, 10)
      .map((r) => `${r.symbol} (${fmtW(r.index_weight)}; ${r.name})`)
      .join("\n");
    ...
    const topAll5d = [...all]
      ...
      .slice(0, 10)
      .map((r) => `${r.symbol} (${fmtPct(r.return_5d)}; ${r.name})`)
      .join("\n");
    ...
    const topAll30d = [...all]
      ...
      .slice(0, 10)
      .map((r) => `${r.symbol} (${fmtPct(r.return_30d)}; ${r.name})`)
      .join("\n");
...
All S&P 500 — biggest risers (5D): ${topAll5d || "(no data)"}
All S&P 500 — biggest risers (30D): ${topAll30d || "(no data)"}
`.trim();

    let prompt = "";
    if (mode === "summary") {
      prompt = `Write a concise S&P 500 market briefing in 3–4 bullet points. Cover: sector rotation, notable movers, and upcoming earnings. No conclusion. No investment advice. If web search is enabled, add recent context with citations.\n\nCONTEXT:\n${context}`;
    } else {
